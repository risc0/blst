...
const vec384 BLS12_381_P BLST_ALIGNED(8) = {    /* (z-1)^2 * (z^4 - z^2 + 1)/3 + z */
...
const radix384 BLS12_381_Rx BLST_ALIGNED(8) = { /* (1<<384)%P, "radix", one-in-Montgomery */
  { { ONE_MONT_P },
    { 0 } }
};

const vec384 BLS12_381_RR BLST_ALIGNED(8) = {   /* (1<<768)%P, "radix"^2, to-Montgomery */
...
const vec256 BLS12_381_r BLST_ALIGNED(8) = {    /* z^4 - z^2 + 1, group order */
    TO_LIMB_T(0xffffffff00000001), TO_LIMB_T(0x53bda402fffe5bfe),
    TO_LIMB_T(0x3339d80809a1d805), TO_LIMB_T(0x73eda753299d7d48)
};

const vec256 BLS12_381_rRR BLST_ALIGNED(8) = {  /* (1<<512)%r, "radix"^2, to-Montgomery */
